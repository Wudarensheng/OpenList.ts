/**
 * 迅雷云盘专家版 (Thunder Expert) Driver — writable
 * Ported from openlistnext (github.com/Polonium-salts/openlistnext)
 * drivers/thunder (ThunderExpertDriver). Self-contained: duplicated pure
 * helpers (MD5/SHA-1/GCID/device-sign) are inlined; no cross-folder driver
 * imports.
 *
 * Supports two login modes (select): username/password or refresh_token.
 * Tokens are kept in instance fields and refreshed in place (no DB
 * persistence) whenever an API call reports an expired access/captcha token.
 */
...
export const thunderExpertConfig: DriverConfig = {
  name: 'ThunderExpert',
  label: '迅雷云盘专家版',
  local_sort: true,
  only_proxy: false,
...
export const thunderExpertAdditional: DriverItem[] = [
  { name: 'root_folder_id', type: 'string', default: '', options: '', required: false, help: '根文件夹 ID' },
  { name: 'login_type', type: 'select', default: 'user', options: 'user,refresh_token', required: true, help: '登录方式' },
  { name: 'sign_type', type: 'select', default: 'algorithms', options: 'algorithms,captcha_sign', required: true, help: '签名方式' },
  { name: 'username', type: 'string', default: '', options: '', required: false, help: 'login type is user, this is required' },
  { name: 'password', type: 'string', default: '', options: '', required: false, help: 'login type is user, this is required' },
  { name: 'refresh_token', type: 'string', default: '', options: '', required: false, help: 'login type is refresh_token, this is required' },
  { name: 'algorithms', type: 'string', default: '9uJNVj/wLmdwKrJaVj/omlQ,Oz64Lp0GigmChHMf/6TNfxx7O9PyopcczMsnf,Eb+L7Ce+Ej48u,jKY0,ASr0zCl6v8W4aidjPK5KHd1Lq3t+vBFf41dqv5+fnOd,wQlozdg6r1qxh0eRmt3QgNXOvSZO6q/GXK,gmirk+ciAvIgA/cxUUCema47jr/YToixTT+Q6O,5IiCoM9B1/788ntB,P07JH0h6qoM6TSUAK2aL9T5s2QBVeY9JWvalf,+oK0AN', options: '', required: false, help: '签名算法列表（逗号分隔）' },
  { name: 'captcha_sign', type: 'string', default: '', options: '', required: false, help: '手动验证码签名' },
  { name: 'timestamp', type: 'string', default: '', options: '', required: false, help: '手动验证码时间戳' },
  { name: 'captcha_token', type: 'string', default: '', options: '', required: false, help: '验证码 token' },
  ...
  { name: 'client_id', type: 'string', default: 'Xp6vsxz_7IYVw2BB', options: '', required: true, help: '客户端 ID' },
  { name: 'client_secret', type: 'string', default: 'Xp6vsy4tN9toTVdMSpomVdXpRmES', options: '', required: true, help: '客户端密钥' },
  { name: 'client_version', type: 'string', default: '8.31.0.9726', options: '', required: true, help: '客户端版本' },
  { name: 'package_name', type: 'string', default: 'com.xunlei.downloadprovider', options: '', required: true, help: '包名' },
  { name: 'user_agent', type: 'string', default: 'ANDROID-com.xunlei.downloadprovider/8.31.0.9726 netWorkType/5G appid/40 deviceName/Xiaomi_M2004j7ac deviceModel/M2004J7AC OSVersion/12 protocolVersion/301 platformVersion/10 sdkVersion/512000 Oauth2Client/0.9 (Linux 4_14_186-perf-gddfs8vbb238b) (JAVA 0)', options: '', required: true, help: 'User-Agent' },
  { name: 'download_user_agent', type: 'string', default: 'Dalvik/2.1.0 (Linux; U; Android 12; M2004J7AC Build/SP1A.210812.016)', options: '', required: true, help: '下载 User-Agent' },
  { name: 'use_video_url', type: 'bool', default: 'false', options: '', required: false, help: '下载优先使用视频源 URL' },
...
const SignProvider = 'access_end_point_token';
const APPID = '40';
...
export class ThunderExpertClient {
...
export class ThunderExpertDriver implements Driver {
  private cfg: Record<string, any> = {};
  private client: ThunderExpertClient | null = null;
  private identity: string = '';

  config(): DriverConfig {
    return thunderExpertConfig;
  }
  ...
  async init(cfg: Record<string, any>): Promise<void> {
    ...
    const algorithms =
      this.cfg.sign_type === 'captcha_sign'
        ? undefined
        : (this.cfg.algorithms || '')
            .split(',')
            .map((s: string) => s.trim())
    ...
    const client = new ThunderExpertClient({
      deviceId,
      clientId: this.cfg.client_id || 'Xp6vsxz_7IYVw2BB',
      clientSecret: this.cfg.client_secret || 'Xp6vsy4tN9toTVdMSpomVdXpRmES',
      clientVersion: this.cfg.client_version || '8.31.0.9726',
      packageName: this.cfg.package_name || 'com.xunlei.downloadprovider',
      userAgent: this.cfg.user_agent || DEFAULT_USER_AGENT,
      downloadUserAgent: this.cfg.download_user_agent || DEFAULT_DOWNLOAD_UA,
      algorithms: algorithms && algorithms.length > 0 ? algorithms : undefined,
      timestamp: this.cfg.timestamp || '',
      captchaSign: this.cfg.captcha_sign || '',
    ...
    let identity = '';
    if (this.cfg.login_type === 'refresh_token') {
      identity = md5(this.cfg.refresh_token || '');
    } else {
      identity = md5(`${this.cfg.username || ''}${this.cfg.password || ''}`);
    }
    ...
    if (this.identity !== identity || !(await client.isLogin())) {
      ...
      if (
        this.cfg.login_type === 'refresh_token' &&
        this.cfg.refresh_token
      ) {
        await client.refreshToken(this.cfg.refresh_token);
      } else if (this.cfg.username && this.cfg.password) {
        await client.login(this.cfg.username, this.cfg.password);
      }
    }
  }

  private requireClient(): ThunderExpertClient {
    if (!this.client) throw new Error('ThunderExpert driver not initialized');
    return this.client;
  }

  private getDownloadUserAgent(): string {
    return this.cfg.download_user_agent || DEFAULT_DOWNLOAD_UA;
  }

  private get useVideoUrl(): boolean {
    return !!(this.cfg.use_video_url === true || this.cfg.use_video_url === 'true');
  }
  ...
  async link(path: string, cfg: Record<string, any>): Promise<LinkResult> {
    ...
    if (this.useVideoUrl && res.medias && res.medias.length > 0) {
    ...
    if (!rawUrl) {
      throw new Error(`ThunderExpert: no download link for ${res.name}`);
    }
...
registerDriver(ThunderExpertDriver, thunderExpertConfig, thunderExpertAdditional);
